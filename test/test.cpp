...
TEST(CanDetectionTest, testTrackCentroid_1) {
  // Load the test image
  cv::Mat image =
      cv::imread(std::string(TEST_SRC_DIR) +
                 "/test2.png");  // Ensure test2.jpg exists in the correct path
  ASSERT_FALSE(image.empty()) << "Error: Unable to load image for testing!";

  // Convert cv::Mat to sensor_msgs::msg::Image
  sensor_msgs::msg::Image image_msg;
  try {
    auto cv_bridge_image = std::make_shared<cv_bridge::CvImage>(
        std_msgs::msg::Header(), "bgr8", image);
    image_msg = *(cv_bridge_image->toImageMsg());  // Dereference the SharedPtr
...
TEST(CanDetectionTest, testTrackCentroid_2) {
  ...
  cv::Mat image =
      cv::imread(std::string(TEST_SRC_DIR) +
                 "/test3.png");  // Ensure test3.jpg exists in the correct path
  ASSERT_FALSE(image.empty()) << "Error: Unable to load image for testing!";
...
TEST(CanDetectionTest, testTrackCentroid) {
  ...
  cv::Mat image =
      cv::imread(std::string(TEST_SRC_DIR) +
                 "/test4.png");  // Ensure test4.jpg exists in the correct path
  ASSERT_FALSE(image.empty()) << "Error: Unable to load image for testing!";
