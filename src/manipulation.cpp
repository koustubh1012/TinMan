...
Manipulation::Manipulation() {
  ...
  delete_client_ = manip_node_->create_client<gazebo_msgs::srv::DeleteEntity>(
      "/delete_entity");

  // Create a client for the spawn service
  spawn_client = manip_node_->create_client<gazebo_msgs::srv::SpawnEntity>(
      "/spawn_entity");
...
void Manipulation::deleteCanEntity(const std::string &entity_name) {
  ...
  if (!delete_client_->wait_for_service(std::chrono::seconds(5))) {
    RCLCPP_ERROR(manip_node_->get_logger(),
                 "Service /delete_entity not available.");
    return;
  }
  ...
  if (rclcpp::spin_until_future_complete(manip_node_, result,
                                         std::chrono::seconds(10)) ==
      rclcpp::FutureReturnCode::SUCCESS) {
    RCLCPP_INFO(manip_node_->get_logger(), "Successfully deleted entity: %s",
                entity_name.c_str());
  } else {
    RCLCPP_ERROR(manip_node_->get_logger(), "Failed to delete entity: %s",
                 entity_name.c_str());
  }
}
...
void Manipulation::spawnCanEntity() {
  ...
  if (!spawn_client->wait_for_service(std::chrono::seconds(5))) {
    RCLCPP_ERROR(manip_node_->get_logger(),
                 "Service /spawn_entity not available.");
    return;
  }

  // Read the SDF file from the specified path
  std::string package_path =
      ament_index_cpp::get_package_share_directory("tinman");
  ...
  if (rclcpp::spin_until_future_complete(manip_node_, result,
                                         std::chrono::seconds(10)) ==
      rclcpp::FutureReturnCode::SUCCESS) {
    RCLCPP_INFO(manip_node_->get_logger(),
                "Successfully spawned can at desired location.");
  } else {
