...
TinMan::TinMan()
    ...
    pose_subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/odom", 10, std::bind(&TinMan::poseCallback, this, std::placeholders::_1));
...
void TinMan::poseCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    if (nav_to_bin) {
        ...
        RCLCPP_INFO(this->get_logger(), "Distance to trash can: %.2f", distance_to_goal);

        if (distance_to_goal <= tolerance) {
            RCLCPP_INFO(this->get_logger(), "Goal reached at x: %.2f, y: %.2f.", goal_x, goal_y);
